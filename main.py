...
print("the currently used version is: ", torch.__version__)

class SQuADDs(Dataset):
...
def evaluate(eval_file, answer_dict):
    ...
    f1 = 100.0 * f1 / total
    return {'exact_match(EM)': exact_match, 'f1': f1}
...
def validation(model, dataset, eval_file):
    model.eval()
    answer_dict = {}
    losses = []
...
def train_entry():
    ...
    train_dataset = SQuADDs(config.train_record_file, config.num_steps, config.batch_size)
    dev_dataset = SQuADDs(config.dev_record_file, config.test_num_batches, config.batch_size)
    ...
    for iter in range(0, N, L):
        ...
        validation(model, train_dataset, train_eval_file)
...
def test_entry():
    with open(config.dev_eval_file, "r") as fh:
        dev_eval_file = json.load(fh)
    dev_dataset = SQuADDs(config.dev_record_file, -1, config.batch_size)
    fn = os.path.join(config.save_dir, "model.pt")
    model = torch.load(fn)
